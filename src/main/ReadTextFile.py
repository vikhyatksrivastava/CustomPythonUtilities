...
def readFile():
    spark_var = SparkSession.builder.appName("WriteDataFrameToHive").getOrCreate()
    path = "C:\\Projects\\postgre\\PostGre_Projects\\scripts\\EmployeeDataAnalysis\\employee_data.csv"
    df = spark_var.read.csv(path, header=True, inferSchema=True)
    # df.printSchema()
    # df.show()
    ...
    average_salary.write.bucketBy(2, "employee_id").sortBy("employee_id").mode("overwrite").saveAsTable(
        "employee_data_bucketed")
    ...
    average_salary.write.option("header", "true").option("sep", ";").option("nullValue", "NA").csv(
        "employee_data_custom.csv")
